...
import Domain.Character.Monsters.MonsterFabric;
import Domain.Character.Monsters.Ogre;
...
public class CharacterList {
  ...
  public void createMonsters(char[][] map, Room[] rooms, int floor, int checkHard, int level) {
    int count = (checkHard == 0) ? 2 + floor / 2 : (checkHard == 1) ? 5 + floor : 5 + floor / 2;
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (level > 3 && checkHard == 1) {
        monsters.get(i).upStats();
      } else if (level > 3 && checkHard == 0) {
  ...
  public void setList(List<Character> monsters) {
    this.monsters = (ArrayList<Character>) monsters;
  }
  ...
  public void printMonsterList(char[][] map) { // не нужно, но пока оставить
    for (Character monster : monsters) {
      map[monster.getPosition().y() + 1][monster.getPosition().x()] = monster.getMonsterName();
    }
  }
