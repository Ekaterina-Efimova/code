...
void sigact(params_t *prms) {
  const action fsm_table[10][8] = {
      {check_timer, exitstate, exitstate, exitstate, exitstate, exitstate,
       exitstate, exitstate},
...
void check_timer(params_t *prms) {
  prms->game_info->pause = 0;
  if (timer(&(prms->timer), get_time_ms(), FALL_DELAY))
    prms->tetris_state = SHIFTING;
  else
    prms->tetris_state = CHECKING;
}
...
void gameover(params_t *prms) {
  ...
  prms->flag = GAMEOVER;
  prms->tetris_state = EXIT_STATE;
  prms->game_info->pause = 2;
  result(prms->game_info, FILE_SCORE);
}

void exitstate(params_t *prms) { prms->tetris_state = EXIT_STATE; }
