...
void bg_game(WINDOW *brick_game_window) {
  ...
  while (action != Terminate) {
    ...
    if (action != -1) userInput((UserAction_t)action, false);

    info = updateCurrentState();
    update_display(brick_game_window, &info);
    if (action != Terminate) action = -1;
    
    if (info.pause == 2)
      while (action != Terminate) {
        print_result(brick_game_window, info.score, info.highscore);
        user_input(brick_game_window, &action, &state);
      }
  }
}
...
void print_result(WINDOW *brick_game_window, int s, int h) {
  for (int i = 16; i < 28; ++i)
    for (int j = 12; j < 31; ++j) print_final_frame(brick_game_window, i, j);

  print_final_info(brick_game_window, s, h);
...
void print_final_info(WINDOW *brick_game_window, int score, int highscore) {
  if (score == highscore)
  ...
  mvwprintw(brick_game_window, 25, 25, "%4d", score);
  mvwprintw(brick_game_window, 26, 14, "%s", bg_objects(INFO, HIGHSCORE));
  mvwprintw(brick_game_window, 26, 25, "%4d", highscore);
}
